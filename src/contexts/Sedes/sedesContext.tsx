...
interface SedesContextType {
    ...
    loading: boolean;
    error: string | null;
    addNewSede: (sede: Sede) => void;
    deleteSedes: (id: number) => void;
}

// eslint-disable-next-line react-refresh/only-export-components
export const SedesContext = createContext<SedesContextType | undefined>(undefined);
...
export const SedesProvider = ({ children }: { children: React.ReactNode }) => {
    ...
    const addNewSede = (sede: Sede) => {
        setSedes("sedes", (prevSedes) => {
            const nextId = (prevSedes.at(-1)?.id ?? 0) + 1;
            return [...prevSedes, { id: nextId, nombre: sede.nombre, direccion: '', telefono: '' }];
        });
    };

    const deleteSedes = (id: number) => {
        setSedes("sedes", (prevGeneros) =>
